...
class DoctorController extends Controller
{
    ...
    public function datatable(Request $request)
    {
        ...
        if (request()->ajax()) {
            return datatables()->of($doctor->orderBy('doctors.id','asc')->get())
    ...
    {
        $patient = Patient::join('doctor_patient', 'doctor_patient.patient_id', '=', 'patients.id')
            ->where('doctor_patient.doctor_id', '=', $request->id)
            ->select('patients.*');

        return Datatables::of($patient->orderBy('doctor_patient.created_at', 'desc'))
            ->addColumn('action', function ($data) {
    ...
    public function select(Request $request)
    {
        ...
        $patient = Patient::leftJoin('doctor_patient', function ($join) use ($doctor) {
            ...
            ->whereNull('doctor_patient.doctor_id');
    ...
    public function deletePatient(Request $request)
    {
        $patient = DoctorPatient::where('doctor_patient.doctor_id', '=', $request->doctor_id)
            ->where('doctor_patient.patient_id', '=', $request->id)
            ->delete();
        if ($patient) {
...
{
    $appointment = Appointment::select('*','patients.first_name as patient')
        ->leftjoin('patients','patients.id','=','appointments.patient_id')
        ->where('appointments.doctor_id','=',$request->id);
